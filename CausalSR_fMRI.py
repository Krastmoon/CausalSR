...
# 读取.mat文件
mat_data = scipy.io.loadmat(r'E:\work\因果推断\code\data\fmri\sim4.mat')  # 替换为实际文件路径

# 查看文件内容
print("文件中的变量:", mat_data.keys())

ts_data = pd.DataFrame(mat_data["ts"])
net_data = mat_data["net"]
print(mat_data["Nnodes"],mat_data["Nsubjects"])
Nsubjects = 50
...
def run_one_experiment(seed=0,
                       ...
                       l1_lambda=1e-3,
                       fit_lambda=1e-1,
                       edge_threshold=0.4,data = None,true = None):
    set_random_seed(seed)
    if band_ranges is None:
        band_ranges = {"Low": (0.1, 6), "Mid": (7, 15), "High": (17, 25)}

    # 合成数据
    variables = [i for i in range(d)]
    dt = 1
    ...
    # 假设 A_est 是包含三个频段邻接矩阵的字典
    A_low = A_est["Low"]
    A_mid = A_est["Mid"]
    A_high = A_est["High"]

    # 逻辑“或”运算，检查是否有边存在
    A_final = np.logical_or(np.logical_or(np.abs(A_low) > edge_threshold, np.abs(A_mid) > edge_threshold), np.abs(A_high) > edge_threshold).astype(int)
    ...
    mse_list, mae_list = [], []
    for i in range(d):
        mse_i, mae_i = evaluate_signal(data[i], X_pred[i])
    ...
    B_true = (true > 0).astype(int)
    print(B_true)
    print(A_final)
    auc_score = roc_auc_score(B_true.flatten(), A_final.flatten())
    acc = count_accuracy_no_dag_check(B_true, A_final)
...
def main():
    ...
    for subject_idx in range(Nsubjects):
        ts_subject = ts_data.iloc[subject_idx * 200: (subject_idx + 1) * 200, :]
        A_true = net_data[subject_idx]
        ts_arr = np.array(ts_subject)
        metrics = run_one_experiment(
            seed=1234 + subject_idx,
            duration=20,
            fs=500,
            d=50,
        ...
        rows.append(metrics)
        print(f"Run {subject_idx}: {metrics}")
